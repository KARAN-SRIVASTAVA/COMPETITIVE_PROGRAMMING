...
long tab(int n,int *coin,int x){
  ...
  for(int j=0;j<=x;j++) if(!(j%coin[0])) dp[0][j] = 1;
  for(int i=0;i<n;i++) dp[i][0] = 1;
  for(int i=1;i<n;i++){
    for(int j=1;j<=x;j++){
      dp[i][j] = dp[i-1][j];
      if(coin[i]<=j) dp[i][j]+=dp[i][j-coin[i]];
    }
  }
  return dp[n-1][x];
}
long Opt(int n,int *coin,int x){
  ...
  for(int j=0;j<=x;j++) if(!(j%coin[0])) prev[j] = 1;
  for(int i=1;i<n;i++){
    vector<long>curr(x+1,0);
    curr[0] = 1;
    for(int j=1;j<=x;j++){
      curr[j] = prev[j];
      if(coin[i]<=j) curr[j]+=curr[j-coin[i]];
    }
    prev = curr;
  }
  return prev[x];
...
long Opt2(int n,int *coin,int x){
  ...
  for(int i=1;i<n;i++){
    prev[0] = 1;
    for(int j=1;j<=x;j++)
      if(coin[i]<=j) prev[j]+=prev[j-coin[i]];
  }
  return prev[x];
}
